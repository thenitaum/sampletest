...
if ( !function_exists('franklin_edd_append_purchase_link') ) {
	...
	function franklin_edd_append_purchase_link() {
		...
		/**
		 * Display amazon products start
		 */
		$campresults = get_post_meta($post->ID);
		$style = '';
		$style.='<style>
			.res-container {
				width: 165px;
		...
		echo $style;
		echo '<p>1. Find a problem in your community that you want to do something about. Describe the problem to us.</p>';
		echo "<blockquote>".$campresults['campaign_q1'][0]."</blockquote>";
		echo '<p>2. You will need to validate that the problem exists with a measure like a survey. Explain how you will do this. (Ex: I will use a survey with 4 questions about the community problem to validate that the problem exists.)</p>';
		echo "<blockquote>".$campresults['campaign_q2'][0]."</blockquote>";
		echo '<p>3. Design a project to address the problem using your skills and passions. (Ex: I want to make a song (because music is my skill) promiting non-violence (because violence is a problem).)</p>';
		echo '<blockquote><p>A) Your idea for the project:'.$campresults['campaign_q3a'][0].'</p>';
		echo '<p>B) The materials:'.$campresults['campaign_q3b'][0]."</p><div style='display:flex;'>";
		
		$amazonproducts = unserialize($campresults['amazonproducts'][0]);
		if(is_array($amazonproducts)){
		foreach($amazonproducts as $row)
		{
			echo '<div class="res-container">';
			echo '<h2><a href="'.$row['amazon_url'].'" target="_blank">'.$row['amazon_title'].'</a></h2>';
			echo '<img src="'.$row['amazon_img'].'">';
			echo '<h3>Price: '.$row['amazon_price'].'</h3>';
			echo '</div>';
		}}
		echo '</div></blockquote><div style="clear:both;"></div>';
		echo '<p>4. Define measures of success so you will know when your project has met your goals. At IdeatEd we are using "shares" to measure impact. When you make a sharable project (music, video, writing, etc.) where you are leading by example, sharing your media has an impact because you are bringing awareness to an issue. Please describe what your share goals are. (Ex: I\'m making a video about bullying and I\'m going to show it to 25 7th graders so they understand how harmfull bullying can be to the person being bullied.)</p>';
		echo "<blockquote>".$campresults['campaign_q4'][0]."</blockquote>";
		
		
		
		/**
		 * Display amazon products end
		 */
