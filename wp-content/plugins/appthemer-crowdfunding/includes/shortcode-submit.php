...
class ATCF_Submit_Campaign {
	...
	public function save_q1( $key, $field, $campaign, $fields ) {
		update_post_meta( $campaign, 'campaign_q1', $field[ 'value' ] );
	}
	public function save_q2( $key, $field, $campaign, $fields ) {
		update_post_meta( $campaign, 'campaign_q2', $field[ 'value' ] );
	}
	public function save_q3a( $key, $field, $campaign, $fields ) {
		update_post_meta( $campaign, 'campaign_q3a', $field[ 'value' ] );
	}
	public function save_q3b( $key, $field, $campaign, $fields ) {
		update_post_meta( $campaign, 'campaign_q3b', $field[ 'value' ] );
	}
	public function save_q4( $key, $field, $campaign, $fields ) {
		update_post_meta( $campaign, 'campaign_q4', $field[ 'value' ] );
	}
...
?>
	...
	<form action="" method="post" class="atcf-submit-campaign" enctype="multipart/form-data">
		<?php //include get_template_directory()."/amazonfiles/amazonmainfile.php";?>
		<?php
			foreach ( atcf_shortcode_submit_fields() as $key => $field ) :
				...
				if($key=="q1")
				{
					...
					echo '<div style="font-weight:bold;">Project Proposal Steps:(Answer these questions in the details section below. These details will show up on the beta.ideated.io website. For each number, short(even one sentence) descriptions are fine. A visualization for the steps below can be found at www.ideated.io under "How IdeatEd Works".</div>';	
					echo '<div style="font-weight:600;">1. Find a problem in your community that you want to do something about. Describe the problem to us.</div>';
				}
				if($key=="q2")
				{
					echo '<div style="font-weight:600;">2. You will need to validate that the problem exists with a measure like a survey. Explain how you will do this. (Ex: I will use a survey with 4 questions about the community problem to validate that the problem exists.)</div>';
				}
				if($key=="q3a")
				{
					echo '<div style="font-weight:bold;">3. Design a project to address the problem using your skills and passions. (Ex: I want to make a song (because music is my skill) promiting non-violence (because violence is a problem).)</div>';
					echo '<div style="font-weight:600;">A) Your idea for the project</div>';
				}
				if($key=="q3b")
				{
					 echo '<div style="font-weight:600;">B) The materials you are requesting. Please check Amazon before for your materials before looking elsewhere. Use this search to upload your Amazon items. If you cant\'n find your items on Amazon, list them in the extra text box below, with prices and links.</div>';
					 include get_template_directory()."/amazonfiles/amazonmainfile.php";
				}
				if($key=="q4")
				{
					echo '<div style="font-weight:600;">4. Define measures of success so you will know when your project has met your goals. At IdeatEd we are using "shares" to measure impact. When you make a sharable project (music, video, writing, etc.) where you are leading by example, sharing your media has an impact because you are bringing awareness to an issue. Please describe what your share goals are. (Ex: I\'m making a video about bullying and I\'m going to show it to 25 7th graders so they understand how harmfull bullying can be to the person being bullied.)</div>';
				}
...
function atcf_submit_process_after( $campaign, $postdata, $status, $fields ) {
	...
	/**
	 * Save amazon products start
 	 */
	 if(isset($_POST['amazonadd']))
	 {
		$amazonpro = $_POST['amazonadd'];
		$amaProducts = array();
		foreach( $amazonpro as $key => $n ) {
			$amaProduct = array();
			$amaProduct['amazon_title'] = $_POST['amazontitle'][$n];
			$amaProduct['amazon_url'] = $_POST['amazonurl'][$n];
			$amaProduct['amazon_img'] = $_POST['amazonimg'][$n];
			$amaProduct['amazon_price'] = $_POST['amazonprice'][$n];
			$amaProducts[] = $amaProduct;
		}
		update_post_meta( $campaign, 'amazonproducts', $amaProducts );
	 }
	/**
	 * Save amazon products end
 	 */  	
